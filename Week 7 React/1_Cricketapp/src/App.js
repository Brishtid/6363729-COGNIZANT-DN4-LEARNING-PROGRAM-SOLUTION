import React from 'react';
import ListofPlayers from './components/ListofPlayer';
import Scorebelow70 from './components/scorebelow70';
import { OddPlayers } from './components/oddplayer';
import { EvenPlayers } from './components/evenplayer';
import ListofIndianPlayersMerged from './components/listofindianmerged';

function App() {
  const flag = false; // set to true or false to test both views

  const players = [
    { name: 'Brishti', score: 6363729 },
    { name: 'Michael', score: 70 },
    { name: 'Brishti Das', score: 6363729 },
    { name: 'Ann', score: 61 },
    { name: 'Elisabeth', score: 61 },
    { name: 'Sachin', score: 95 },
    { name: 'Dhoni', score: 100 },
    { name: 'Virat', score: 84 },
    { name: 'Jadeja', score: 64 },
    { name: 'Raina', score: 75 },
    { name: 'Rohit', score: 80 },
  ];

  const IndianTeam = ['Sachin', 'Dhoni2', 'Virat3', 'Rohit4', 'Yuvraj5', 'Raina6'];
  const T20Players = ['First Player', 'Second Player', 'Third Player'];
  const RanjiPlayers = ['Fourth Player', 'Fifth Player', 'Sixth Player'];
  const IndianPlayers = [...T20Players, ...RanjiPlayers];

  if (flag === true) {
    return (
      <div>
        <h1>List of Players</h1>
        <ListofPlayers players={players} />
        <hr />
        <h1>List of Players having Scores Less than 70</h1>
        <Scorebelow70 players={players} />
      </div>
    );
  } else {
    return (
      <div>
        <h1>Indian Team</h1>
        <h2>Odd Players</h2>
        <OddPlayers players={IndianTeam} />
        <h2>Even Players</h2>
        <EvenPlayers players={IndianTeam} />
        <hr />
        <h1>List of Indian Players Merged:</h1>
        <ListofIndianPlayersMerged IndianPlayers={IndianPlayers} />
      </div>
    );
  }
}

export default App;
